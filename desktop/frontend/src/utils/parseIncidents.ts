...
function extractTitle(text: string): string | null {
  // "Error: ...", "Issue: ...", "Alert: ..."
  const errorMatch = text.match(/(?:^|\n)\s*[-*]?\s*\*{0,2}(?:Error|Issue|Alert|Problem|Incident)\s*:?\*{0,2}\s*:?\s*(.+)/im);
  ...
  if (headingMatch) {
    ...
    if (!/^(incident summary|code analysis|root cause|timeline|affected|references|recommended|customer impact|investigation|monitoring check)/i.test(heading)) {
...
function parseMessageForIncidents(message: Message): ParsedIncident[] {
  ...
  // Pattern 3: ### Incident Summary blocks
  const summaryPattern = /#{1,3}\s*Incident Summary\s*\n([\s\S]*?)(?=\n#{1,3}\s|\n---|\n\*\*\*|$)/gi;
  while ((match = summaryPattern.exec(content)) !== null) {
    const block = match[1];
    const titleMatch = block.match(/(?:Title|Issue|Error)\s*:\s*(.+)/i);
  ...
  if (incidents.length === 0) {
    ...
    const hasIncidentKeywords = /customer impact|affected systems|deployment issue|investigation|error rate|alert/i.test(content);
