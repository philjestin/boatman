...
// Handoff is the interface for structured context passing.
...
// Runner runs tests for the project.
type Runner struct {
	worktreePath string
}

// New creates a new test runner.
func New(worktreePath string) *Runner {
	return &Runner{
...
func (r *Runner) DetectFramework() (*Framework, error) {
	// Check for Go
	if _, err := os.Stat(filepath.Join(r.worktreePath, "go.mod")); err == nil {
		return &Framework{
			Name:        "go",
			Command:     "go",
	...
	if _, err := os.Stat(filepath.Join(r.worktreePath, "Gemfile")); err == nil {
		// Check for rspec
		gemfile, _ := os.ReadFile(filepath.Join(r.worktreePath, "Gemfile"))
	...
	// Check for Node.js
	if _, err := os.Stat(filepath.Join(r.worktreePath, "package.json")); err == nil {
		pkgJSON, _ := os.ReadFile(filepath.Join(r.worktreePath, "package.json"))
	...
	// Check for Python
	if _, err := os.Stat(filepath.Join(r.worktreePath, "pytest.ini")); err == nil {
		return &Framework{
			Name:        "pytest",
			Command:     "pytest",
			Args:        []string{"-v", "--cov"},
	...
	if _, err := os.Stat(filepath.Join(r.worktreePath, "setup.py")); err == nil {
		return &Framework{
			Name:        "pytest",
			Command:     "pytest",
			Args:        []string{"-v"},
	...
	if _, err := os.Stat(filepath.Join(r.worktreePath, "pyproject.toml")); err == nil {
		return &Framework{
			Name:        "pytest",
			Command:     "pytest",
			Args:        []string{"-v"},
...
func (r *Runner) RunAll(ctx context.Context) (*TestResult, error) {
	framework, err := r.DetectFramework()
	if err != nil {
		return &TestResult{
	...
	return r.runTests(ctx, framework, framework.Args)
}

// RunForFiles runs tests relevant to specific changed files.
func (r *Runner) RunForFiles(ctx context.Context, changedFiles []string) (*TestResult, error) {
	framework, err := r.DetectFramework()
	if err != nil {
		return &TestResult{
			Passed: true,
	...
	testFiles := r.FindRelatedTests(changedFiles, framework)
	if len(testFiles) == 0 {
	...
	args := r.buildTargetedArgs(framework, testFiles)
	return r.runTests(ctx, framework, args)
}

// FindRelatedTests finds test files related to changed files.
func (r *Runner) FindRelatedTests(changedFiles []string, framework *Framework) []string {
	var testFiles []string
	seen := make(map[string]bool)

	for _, file := range changedFiles {
		// If it's already a test file, include it
		if r.IsTestFile(file, framework) {
			if !seen[file] {
		...
		testFile := r.FindTestFile(file, framework)
...
func (r *Runner) IsTestFile(file string, framework *Framework) bool {
...
func (r *Runner) FindTestFile(file string, framework *Framework) string {
	...
	for _, candidate := range candidates {
		fullPath := filepath.Join(r.worktreePath, candidate)
...
func (r *Runner) buildTargetedArgs(framework *Framework, testFiles []string) []string {
	switch framework.Name {
	case "go":
...
func (r *Runner) runTests(ctx context.Context, framework *Framework, args []string) (*TestResult, error) {
	start := time.Now()
	...
	r.parseOutput(result, output, framework)
...
func (r *Runner) parseOutput(result *TestResult, output string, framework *Framework) {
	switch framework.Name {
	case "go":
		r.parseGoOutput(result, output)
	case "rspec":
		r.parseRspecOutput(result, output)
	case "jest", "vitest":
		r.parseJestOutput(result, output)
	case "pytest":
		r.parsePytestOutput(result, output)
	default:
...
func (r *Runner) parseGoOutput(result *TestResult, output string) {
	result.Passed = true
...
func (r *Runner) parseRspecOutput(result *TestResult, output string) {
...
func (r *Runner) parseJestOutput(result *TestResult, output string) {
...
func (r *Runner) parsePytestOutput(result *TestResult, output string) {
...
// Execute runs tests based on the provided handoff context.
func (r *Runner) Execute(ctx context.Context, handoff Handoff) (Handoff, error) {
	filesHandoff, ok := handoff.(*FilesHandoff)
	if ok && len(filesHandoff.Files) > 0 {
		result, err := r.RunForFiles(ctx, filesHandoff.Files)
		if err != nil {
			return nil, err
		}
