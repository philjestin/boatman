...
// Verifier verifies diffs against issues.
type Verifier struct {
	worktreePath          string
	minConfidenceOverride int // Optional minimum confidence override
}

// New creates a new diff verifier.
func New(worktreePath string) *Verifier {
	return &Verifier{
...
func (v *Verifier) SetMinConfidence(minConfidence int) {
	v.minConfidenceOverride = minConfidence
}

// Verify checks if the diff addresses the given issues.
func (v *Verifier) Verify(ctx context.Context, issues []review.Issue, oldDiff, newDiff string) (*VerificationResult, error) {
	...
	for _, issue := range issues {
		addressed, evidence, reason := v.checkIssueAddressed(issue, oldChanges, newChanges)
	...
	result.NewIssues = v.detectNewIssues(oldChanges, newChanges)
...
func (v *Verifier) checkIssueAddressed(issue review.Issue, oldChanges, newChanges map[string]*DiffChange) (bool, string, string) {
	// Strategy:
	...
	if issue.File != "" {
		...
		evidence := v.findFixEvidence(newChange, keywords, issue)
		if evidence != "" {
	...
	for file, change := range newChanges {
		...
		evidence := v.findFixEvidence(change, keywords, issue)
		if evidence != "" {
	...
	if removal := v.checkPatternRemoved(oldChanges, newChanges, issue); removal != "" {
		return true, removal, ""
	}
...
func (v *Verifier) findFixEvidence(change *DiffChange, keywords []string, issue review.Issue) string {
...
func (v *Verifier) checkPatternRemoved(oldChanges, newChanges map[string]*DiffChange, issue review.Issue) string {
...
func (v *Verifier) detectNewIssues(oldChanges, newChanges map[string]*DiffChange) []string {
	var issues []string
...
// Execute runs verification using a Handoff for structured context passing.
func (v *Verifier) Execute(ctx context.Context, handoff Handoff) (Handoff, error) {
	verifyHandoff, ok := handoff.(*VerifyHandoff)
	if !ok {
		return nil, fmt.Errorf("expected VerifyHandoff, got %T", handoff)
	}

	result, err := v.Verify(ctx, verifyHandoff.Issues, verifyHandoff.OldDiff, verifyHandoff.NewDiff)
	if err != nil {
		return nil, err
	}
...
func (h *VerificationHandoff) Full() string {
	...
	if len(r.NewIssues) > 0 {
		sb.WriteString("## Potential New Issues\n")
