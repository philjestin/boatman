...
// FileLock provides file locking capabilities for multi-agent scenarios.
type FileLock interface {
	LockFiles(ownerID string, files []string) bool
	UnlockFiles(ownerID string, files []string)
}
...
// SetFileLock sets the file lock implementation for file locking.
func (cp *ContextPinner) SetFileLock(fl FileLock) {
	cp.fileLock = fl
}
...
func (cp *ContextPinner) Pin(agentID string, files []string, lock bool) (*Pin, error) {
	...
	// Try to lock files if a file lock is configured
	if lock && cp.fileLock != nil {
		if !cp.fileLock.LockFiles(agentID, fileList) {
...
func (cp *ContextPinner) Unpin(agentID string) {
	...
	if ok && pin.Locked && cp.fileLock != nil {
		cp.fileLock.UnlockFiles(agentID, pin.Files)
	}
}
