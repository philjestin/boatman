...
When Datadog evidence identifies a high-confidence remediation, it also asks
Claude Code to use `/plan`, create a fresh worktree from latest main, implement
the smallest safe fix, run targeted validation, request `/peer-review` and
`/lydia-code-review`, and address actionable feedback. If evidence is missing
or ambiguous, it stops after the investigation report.
