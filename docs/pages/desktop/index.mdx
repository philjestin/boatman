...
Boatman Desktop is a native desktop application built with Wails (Go + React) that provides a rich GUI for interacting with AI agents on your codebase, with specialized **Firefighter Mode** for production incident investigation and a Runtime inspector for provider-neutral runs and memory.
