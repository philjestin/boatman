...
- `plan`: model for `/plan` and planning phases
- `implementation`: model for code changes and refactor work
- `skills`: model for review skills and other skill execution

Any empty field falls back to the model selected in the Routines tab. The
Datadog remediation loop passes the resolved profile into each child
BoatmanMode run.
...
- a machine-readable `boatman_remediation_candidates` block
- automatic remediation summary with branch and draft PR links when Boatman finds credible fixes
...
5. Enter graph area, top N, lookback, environment, optional service, and maximum remediations.
6. Click **Run Routine**.
