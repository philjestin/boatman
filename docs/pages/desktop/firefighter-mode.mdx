...
| Pattern | Example |
|---------|---------|
| Emoji ticket alerts | `🎫 [Urgent] Linear ticket EMP-456: Payment NPE` |
...
- `"I'm on call for the employer side of the handshake monolith. Only care about employer-graphql, promotion-service, and the employer squad's Datadog dashboards."`
- `"Student-facing services only — handshake-web, student-graphql, career-fairs. Ignore employer stuff."`
...
This means the agent won't waste time investigating a student-service Datadog alert when you're only on call for employer services.
...
4. **Reports** — Summarizes findings in the chat:
   - `🎫 [Urgent] Linear ticket EMP-456: Payment refund NPE`
...
|--------|--------------|---------|----------|
| **Datadog plugin** | `claude /plugin install datadog@handshake-marketplace` | Logs, metrics, monitors, traces | Yes |
...
- Describe your on-call scope in natural language: `"employer side of the monolith"`, `"payment and billing services"`
