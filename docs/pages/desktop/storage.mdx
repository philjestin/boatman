...
Enable project-local recording with `BOATMAN_RUNTIME_STORE=1`, or set
`BOATMAN_RUNTIME_STORE_DIR` to write runs somewhere else.
...
They are intended to be human-inspectable. Boatman can load them as context for
future sessions, and the Runtime tab shows their scope, provenance, source run,
expiration, path, and full body.
