...
| **runner/observer** | Run lifecycle event interface (`OnRunStart`, `OnRunComplete`, `OnStepStart`, `OnStepComplete`) |
| **runner/guard** | Mid-run policy enforcement interface (`AllowStep` with `GuardState`) |
...
| **memory/provider** | `MemoryProvider` interface for pluggable memory backends |
...
│   ├── observer.go  # Observer interface for lifecycle events
│   └── guard.go     # Guard interface for mid-run policy checks
│
...
The [Platform module](/platform) implements several harness interfaces, connecting the standalone primitives to organizational infrastructure:

| Harness Interface | Platform Implementation | Purpose |
|-------------------|------------------------|---------|
| `runner.Observer` | `ObserverAdapter` | Publishes run lifecycle events to the event bus |
| `runner.Guard` | `PolicyGuard` | Enforces cost and file limits mid-run |
| `memory.MemoryProvider` | `PlatformMemoryStore` | Provides hierarchically merged memory from org/team/repo scopes |
| `runner.Hooks` | `HooksAdapter`, `CostHooks` | Records step events and cost data |

This integration is transparent — the runner doesn't know whether it's using local or platform-backed implementations.
